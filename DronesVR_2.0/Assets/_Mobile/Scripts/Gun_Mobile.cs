...
public class Gun_Mobile : MonoBehaviour {
    ...
    void Update()
    {
        ...
        for (int i = 0; i < count; i++) {
			...
			if (shoot_gui.HitTest(touch.position) & touch.phase == TouchPhase.Began & touch.phase != TouchPhase.Stationary & _rateofSpeed > RateOfSpeed & (svumode == 0||svumode == 3) & curAmmo > 0)//if shoot
            {
                timeout = 0;
        ...
        {
            camera1.GetComponent<Camera>().fieldOfView = 60;// aim of
            aim = false;                                    //
