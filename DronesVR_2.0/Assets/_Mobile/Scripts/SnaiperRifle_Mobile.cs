...
public class SnaiperRifle_Mobile : MonoBehaviour
{
    ...
    void Update()
    {
        _ON_ = _AimOn.name;     //
        _OFF_ = _AimOff.name;   //
        _gunon_ = _GunOn.name;  // get name of animation
        _idle_ = _Idle.name;    //
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (shoot_gui.HitTest(touch.position) & touch.phase == TouchPhase.Began & touch.phase != TouchPhase.Stationary & _rateofSpeed > RateOfSpeed & svumode != 4 & svumode != 6 & curAmmo > 0)// if shoot
            {
                timeout = 0;
            ...
            if (aim_gui.HitTest(touch.position) & aim == false & svumode == 0)// if aim on 
            {
                svumode = 5;
            }
            if (aim_gui.HitTest(touch.position) & aim == true)//if aim off
            {
                svumode = 6;
            }
        }
